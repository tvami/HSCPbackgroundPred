...
def test_FTest(poly1, poly2, SRorCR='CR', signal='HSCPgluino-1800-125'):
    '''
    ...
    params1 = twoD.ledger.select(_select_signal, 'HSCPgluino-{}'.format(signal), poly1).alphaParams
    rpfSet1 = params1[params1["name"].str.contains("rpf")]
    nRpfs1  = len(rpfSet1.index)
    _gof_for_FTest(twoD, 'HSCPgluino-1800{}_area'.format(poly1), card_or_w='card.txt')
    gofFile1 = working_area+'/HSCPgluino-1800{}_area/higgsCombine_gof_data.GoodnessOfFit.mH120.root'.format(poly1)

    # Get number of RPF params and run GoF for poly2
    params2 = twoD.ledger.select(_select_signal, 'HSCPgluino-{}'.format(signal), poly2).alphaParams
    rpfSet2 = params2[params2["name"].str.contains("rpf")]
    nRpfs2  = len(rpfSet2.index)
    _gof_for_FTest(twoD, 'HSCPgluino-1800{}_area'.format(poly2), card_or_w='card.txt')
    gofFile2 = working_area+'/HSCPgluino-1800{}_area/higgsCombine_gof_data.GoodnessOfFit.mH120.root'.format(poly2)
