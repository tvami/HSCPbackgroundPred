...
for i in range(0, f.GetListOfKeys().GetEntries()):
  ...
  if any(condition in dirname for condition in ["pass", "prefit", "projx0", "projx1", "projy", "_2D", "Background_"]) :
    continue
  if not any(condition in dirname for condition in ["Signal"]) :
    continue

  objData = f.Get("data_obs_fail_postfit_projx2")
  objBkg = f.Get("TotalBkg_fail_postfit_projx2")
  ...
  with open('outputFail.tex', 'a') as file:
